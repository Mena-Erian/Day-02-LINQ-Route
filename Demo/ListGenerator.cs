...
namespace Demo
{
    class ProductEquiltyComparerById : IEqualityComparer<Product>
    {
    ...
    class ProductEquiltyComparerByCategory : IEqualityComparer<Product>
    {
        ...
        public bool Equals(Product? x, Product? y)
        {
            ...
            return string.Equals(x.Category, y.Category, StringComparison.CurrentCulture);
            /// StringComparison.Ordinal =>(pure binary comparer) faster , used in Technical comparisons
            /// StringComparison.CurrentCulture => (Compare based on cultur like in Germent ~ B == ss in some context ) slower // Used in User-facing, localized comparisons
            /// Have Issiues => //return x.Category?.Equals(y.Category is null ? y.Category : "") ?? (y.Category is null ? true : false);
        }

        public int GetHashCode(Product obj)
        {
            return obj.Category?.GetHashCode() ?? 0;
        }
    }
    ...
    class Product : IComparable<Product>, IEquatable<Product>
    {
        ...
        public override int GetHashCode()
         => HashCode.Combine(ProductId, ProductName, Category, UnitPrice, UnitsInStock);
        ...
        static ListGenerator()
        {
            ProductsList = new List<Product>()
            {
                //new Product02(){ SerialNumber = 2323423},
                      new Product{ ProductId = 3, ProductName = "Aniseed Syrup", Category = "Hamada",
